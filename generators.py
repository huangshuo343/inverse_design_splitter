...
def read_complex_data(vol_name, **load_params_fea_complex):
    x_b_map_ima = py.utils.load_volfile(vol_name, **load_params_fea_complex)
...
def read_complex_data_aim(vol_name, **load_params_fea_complex_aim):
    x_b_map_ima = py.utils.load_volfile(vol_name, **load_params_fea_complex_aim)
    x_b_map_ima_shape = x_b_map_ima.shape
    #x_b_map_ima_shape[-1] = 1
    x_b_map_ima_complex = np.zeros((*x_b_map_ima_shape[ : -1], 2), dtype='complex_')
    x_b_map_ima_complex[0, :, :, 0] = x_b_map_ima[0, :, :, 0] - x_b_map_ima[0, :, :, 1] * 1j
    x_b_map_ima_complex[0, :, :, 1] = x_b_map_ima[0, :, :, 2] - x_b_map_ima[0, :, :, 3] * 1j
    #x_b_map_ima_complex[0, :, :, 1] = x_b_map_ima[0, :, :, 0] + x_b_map_ima[0, :, :, 1] * 1j
    return x_b_map_ima_complex

def read_real_data(vol_name, **load_params_fea_real):
    x_real_map_ima = py.utils.load_volfile(vol_name, **load_params_fea_real)
    x_real_map_ima_shape = x_real_map_ima.shape
    #x_b_map_ima_shape[-1] = 1
    x_real_map_ima_reshape = np.zeros([1, 48, 48, 1])
    if len(x_real_map_ima_shape) == 3:
        x_real_map_ima_reshape[0, :, :, 0] = x_real_map_ima[0, :, :]
    else:
        x_real_map_ima_reshape[0, :, :, 0] = x_real_map_ima[0, : , : , 0]
    return x_real_map_ima_reshape
...
def volgen_FOD(
        ...
        pad_shape_dat=None,
        pad_shape_fea=None,
        pad_shape_fea_complex=None,
        pad_shape_fea_complex_aim=None,
        resize_factor=1,
        add_feat_axis=True,
        num_feature_data=1,
        num_feature_feature=1,
        num_feature_complex=2
):
    """
    ...
    while True:
        ...
        load_params_img = dict(np_var=np_var, add_batch_axis=True, num_feature=num_feature_data, pad_shape=pad_shape_dat,
                               resize_factor=resize_factor)
        forward_map_image = [read_real_data(vol_names[i, 0], **load_params_img) for i in indices]
        #print("The data chosen is: ", vol_names[indices[0], 0], ".")
        inverse_map_image = [read_real_data(vol_names[i, 1], **load_params_img) for i in indices]
        load_params_fea = dict(np_var=np_var, add_batch_axis=True, num_feature=num_feature_feature, pad_shape=pad_shape_fea,#num_feature=1 #1
                               resize_factor=resize_factor)
        n_fix_map_image = [read_real_data(vol_names[i, 2], **load_params_fea) for i in indices]  #
        simulation_region_map_image = [read_real_data(vol_names[i, 3], **load_params_fea) for i in indices]
        load_params_fea_complex = dict(np_var=np_var, add_batch_axis=True, num_feature=num_feature_complex,
                                       pad_shape=pad_shape_fea_complex, resize_factor=resize_factor)
        x_b_map_image = [read_complex_data(vol_names[i, 4], **load_params_fea_complex) for i in indices]  #
        load_params_fea_complex_aim = dict(np_var=np_var, add_batch_axis=True, num_feature=num_feature_complex * 2,
                                       pad_shape=pad_shape_fea_complex_aim, resize_factor=resize_factor)
        y_aim_map_image = [read_complex_data_aim(vol_names[i, 5], **load_params_fea_complex_aim) for i in indices]
        # imgs2 = [py.utils.load_volfile(vol_names[i, 0], **load_params) for i in indices]
        #imgs3 = [py.utils.load_volfile(vol_names[i, 2], **load_params_fea) for i in indices]  #
        #feature2 = [py.utils.load_volfile(vol_names[i, 3], **load_params_fea) for i in indices]  #
        #feature3 = [py.utils.load_volfile(vol_names[i, 4], **load_params_fea) for i in indices]  ##
        #feature3 = [py.utils.load_volfile(vol_names[i, 5], **load_params_fea) for i in indices]

        vols = [[forward_map_image[i], inverse_map_image[i], n_fix_map_image[i], simulation_region_map_image[i],
                 x_b_map_image[i], y_aim_map_image[i]]
                for i in range(len(indices))]# imgs3[i], #vols = [[imgs1[i], imgs2[i], imgs3[i], feature1[i], feature2[i]] for i in range(len(indices))] , feature2[i] #, imgs3[i] #, feature2[i], feature3[i]

        load_params_img['np_var'] = 'forward_map_ima'  # be sure to load seg
        forward_map_ima = [read_real_data(vol_names[i, 0], **load_params_img) for i in indices]
        vols.append(np.concatenate(forward_map_ima, axis=0))

        load_params_img['np_var'] = 'inverse_map_ima'  # be sure to load seg
        inverse_map_ima = [read_real_data(vol_names[i, 1], **load_params_img) for i in indices]
        vols.append(np.concatenate(inverse_map_ima, axis=0))

        load_params_img['np_var'] = 'n_fix_map_ima'  # be sure to load seg #T1_ima
        n_fix_map_ima = [read_real_data(vol_names[i, 2], **load_params_fea) for i in indices] #T1_ima
        vols.append(np.concatenate(n_fix_map_ima, axis=0)) #T1_ima

        load_params_img['np_var'] = 'simulation_region_map_ima'  # be sure to load seg #T1_ima
        simulation_region_map_ima = [read_real_data(vol_names[i, 3], **load_params_fea) for i in indices]  # T1_ima
        vols.append(np.concatenate(simulation_region_map_ima, axis=0))  # T1_ima

        load_params_img['np_var'] = 'x_b_map_ima'  # be sure to load seg #T1_ima
        x_b_map_ima_complex_all = [read_complex_data(vol_names[i, 4], **load_params_fea_complex) for i in indices]  # T1_ima
        vols.append(np.concatenate(x_b_map_ima_complex_all, axis=0))  # T1_ima


        load_params_fea['np_var'] = 'y_aim_map_ima'  # be sure to load seg
        # neg_ima = [py.utils.load_volfile(vol_names[i, 1], **load_params) for i in indices]
        y_aim_map_ima_complex_all = [read_complex_data_aim(vol_names[i, 5], **load_params_fea_complex_aim) for i in indices]  # /
        vols.append(np.concatenate(y_aim_map_ima_complex_all, axis=0))

        yield tuple(vols)


def scan_to_scan_FOD(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False,
                     pad_shape_dat=None, pad_shape_fea=None, pad_shape_complex=None, pad_shape_complex_aim=None,
                     **kwargs):
    """
    ...
    """
    zeros = None
    gen = volgen_FOD(vol_names, pad_shape_dat=pad_shape_dat, pad_shape_fea=pad_shape_fea,
                     pad_shape_fea_complex=pad_shape_complex, pad_shape_fea_complex_aim=pad_shape_complex_aim,
                     batch_size=batch_size, **kwargs)
    while True:
        forward_map, inverse_map, n_fix_map, simulation_region_map, x_b_map, y_aim_map = next(gen)[0]#image_t1, #scan1, image_t1, scan2, feature1, feature2 = next(gen)[0] image_t1, , feature2 # scan1_S0_ln, #, scan1_ln, scan2_ln
        #diff_shape = x_b_map.shape[1 : ]#diff_shape = forward_map.shape[1:] #diff_shape = n_fix_map.shape[1]
        diff_shape = [x_b_map.shape[1], x_b_map.shape[2], x_b_map.shape[3]] # x_b_map.shape[3] = 2
        diff = np.zeros((batch_size, *diff_shape)) #diff = np.zeros((batch_size, *diff_shape)) /
        # cache zeros
        if not no_warp and zeros is None:
            shape = forward_map.shape[1:-1] #shape = forward_map.shape[1:-1] -1
            zeros = np.zeros((batch_size, *shape))#zeros = np.zeros((batch_size, *shape, len(shape))) , len(shape)

        invols = [forward_map, inverse_map, n_fix_map, simulation_region_map, x_b_map, y_aim_map]#image_t1, #image_t1, , feature2 #, scan1_S0_ln #, scan1_ln, scan2_ln
        outvols = [diff, zeros]

        yield (invols, outvols)


def scan_to_scan_raw(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    Generator for scan-to-scan registration.
...
def scan_to_atlas(vol_names, atlas, bidir=False, batch_size=1, no_warp=False, **kwargs):
    ...
    while True:
        scan = next(gen)[0]
        print('shape of atlas', atlas.shape)
