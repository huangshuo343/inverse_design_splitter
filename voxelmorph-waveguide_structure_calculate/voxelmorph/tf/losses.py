...
class Transmit_percentage:
    """
    Transmitted energy of waveguide, using the negative value.
    """
    ...
    def loss(self, y_true, y_pred):
        #return K.abs(1.0 - K.square(K.abs(K.sum(y_true - y_pred))))
        output_channel1 = K.abs(K.square(K.abs(K.sum(y_pred[ : , : , : , 0])))) / (K.abs(K.sum(y_pred[ : , : , : , 2]))
                                                                                   + 0.000001)
        error_channel1 = K.abs(K.sum(y_pred[ : , : , : , 2])) - output_channel1
        output_channel2 = K.abs(K.square(K.abs(K.sum(y_pred[ : , : , : , 1])))) / (K.abs(K.sum(y_pred[ : , : , : , 3]))
                                                                                   + 0.000001)
        error_channel2 = K.abs(K.sum(y_pred[ : , : , : , 3])) - output_channel2
        return abs(error_channel1 + error_channel2) + abs(error_channel1) + abs(error_channel2)
        # return K.abs(1.0 - (K.square(K.abs(K.sum(y_true - y_pred)))))


# class Continuous_energy:
#     '''
#     Continuous loss, the energy in discontinuous region is small.
#     '''
#
#     def __init__(self, image_sigma=1.0):
#         self.image_sigma = image_sigma
